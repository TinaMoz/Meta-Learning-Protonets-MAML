...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            def compute_log_probabilities(distances):
                """Compute log probabilities based on distances."""
            ...
            prototypes = compute_prototypes(support_embeddings, labels_support)

            distances_support = compute_distances(support_embeddings, prototypes)
            distances_query = compute_distances(query_embeddings, prototypes)

            log_p_y_support = compute_log_probabilities(distances_support)
            log_p_y_query = compute_log_probabilities(distances_query)

            loss_support = F.cross_entropy(log_p_y_support, labels_support)
            loss_query = F.cross_entropy(log_p_y_query, labels_query)
            loss = loss_support + loss_query

            acc_support = util.score(log_p_y_support, labels_support)
            acc_query = util.score(log_p_y_query, labels_query)
