...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        # if train:
        ### START CODE HERE ###
       # Compute accuracy before any adaptation
        logits = self._forward(images, parameters)
        accuracy_pre_adapt = util.score(logits, labels)
        accuracies.append(accuracy_pre_adapt)
        print(f'Accuracy before adaptation: {accuracy_pre_adapt:.4f}')  # Отлад
        ...
        for step in range(self._num_inner_steps):
            ...
            # Update parameters using gradients and inner learning rate
            parameters = {
            ...
            logits = self._forward(images, parameters)
            accuracy_post_adapt = util.score(logits, labels)
            accuracies.append(accuracy_post_adapt)
            print(f'Accuracy after step {step + 1} of adaptation: {accuracy_post_adapt:.4f}')  # Отлад
    ...
    def train(self, dataloader_train, dataloader_val, writer, args):
        ...
        for i_step, task_batch in enumerate(
            ...
            outer_loss, accuracies_support, accuracy_query = (
                ...
                print(f"task_batch: {task_batch}")
                print(f"outer_loss_batch: {outer_loss_batch}")
                if len(outer_loss_batch) == 0:
                    print("Error: outer_loss_batch is empty")
            )
            outer_loss.backward()
