...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # Compute support embeddings without gradients
            with torch.no_grad():
                support_embeddings = self._network(images_support)
            
            # Compute query embeddings with gradients
            query_embeddings = self._network(images_query)

            # Compute prototypes without gradients
            with torch.no_grad():
            ...
            # Use distances as logits directly for cross-entropy loss
    ...
    def train(self, dataloader_train, dataloader_val, writer, args):
        ...
        for i_step, task_batch in enumerate(
            ...
            if len(loss_batch) == 0:
                print("Error: loss_batch is empty")
            loss.backward()
